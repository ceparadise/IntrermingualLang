...
class CrossLDA(Inferencer):
    ...
    def random_initialize(self):
        ...
        for doc_id in range(self._number_of_documents_en):
            self._k_dn_en[doc_id] = np.zeros(len(self._word_idss_en[doc_id]))
            for word_pos in range(len(self._word_idss_en[doc_id])):
        ...
        for doc_id in range(self._number_of_documents_cn):
            self._k_dn_cn[doc_id] = np.zeros(len(self._word_idss_cn[doc_id]))
            for word_pos in range(len(self._word_idss_cn[doc_id])):
    ...
    def update_psi(self, ):
        term1 = (1 - self.lam) * self._n_kv_en / (
        np.dot(self.psi_cn, self._trans_cn_en.T) * self.lam + (1 - self.lam) * self.psi_en)
        term2 = self.lam * np.dot(self._n_kv_cn, self._trans_en_cn) / (np.sum(np.dot(self.psi_en, self._trans_en_cn.T),
                                                                              axis=1) * self.lam + (
                                                                       1 - self.lam) * np.sum(self.psi_cn, axis=1))[:,
                                                                      np.newaxis]
        ...
        term1 = (1 - self.lam) * self._n_kv_cn / (
        np.dot(self.psi_en, self._trans_en_cn.T) * self.lam + (1 - self.lam) * self.psi_cn)
        term2 = self.lam * np.dot(self._n_kv_en, self._trans_cn_en) / (np.sum(np.dot(self.psi_cn, self._trans_cn_en.T),
                                                                              axis=1) * self.lam + (
                                                                       1 - self.lam) * np.sum(self.psi_en, axis=1))[:,
                                                                      np.newaxis]
    ...
    def sample_document_en(self, doc_id, local_parameter_iteration=1):
        for iter in range(local_parameter_iteration):
            for position in range(len(self._word_idss_en[doc_id])):
    ...
    def perplexity(self, ):
        ...
        topic_word_probability_en = (self._n_kv_en + self._alpha_beta_en) / (np.sum(self._alpha_beta_en) + np.sum(
            self._n_kv_en, axis=1))[:, np.newaxis]
        for doc_id in range(self._number_of_documents_en):
            doc_topic_probability_en = (self._n_dk_en[doc_id] + self._alpha_alpha) / (
            np.sum(self._n_dk_en[doc_id]) + np.sum(self._alpha_alpha))
            word_count_en += len(self._word_idss_en[doc_id])
            for pos in range(len(self._word_idss_en[doc_id])):
        ...
        topic_word_probability_cn = (self._n_kv_cn + self._alpha_beta_cn) / (np.sum(self._alpha_beta_cn) + np.sum(
            self._n_kv_cn, axis=1))[:, np.newaxis]
        for doc_id in range(self._number_of_documents_cn):
            doc_topic_probability_cn = (self._n_dk_cn[doc_id] + self._alpha_alpha) / (
            np.sum(self._n_dk_cn[doc_id]) + np.sum(self._alpha_alpha))
            word_count_cn += len(self._word_idss_cn[doc_id])
            for pos in range(len(self._word_idss_cn[doc_id])):
    ...
    def learning(self):
        ...
        print("iteration %i finished in %d seconds with training perplexity %g" % (
        self._counter, processing_time, self.perplexity()))
    ...
    def export_document(self, exp_beta_path):
        ...
        doc_topic_probability_en = (self._n_dk_en + self._alpha_alpha) / (np.sum(self._n_dk_en, axis=1) + np.sum(
            self._alpha_alpha))[:, np.newaxis]
        doc_topic_probability_cn = (self._n_dk_cn + self._alpha_alpha) / (np.sum(self._n_dk_cn, axis=1) + np.sum(
            self._alpha_alpha))[:, np.newaxis]
