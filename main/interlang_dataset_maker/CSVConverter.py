...
def get_csv_writing_path(source_root_path):
    """
    ...
    parent_dir = os.path.dirname(source_root_path)
    csv_dir_path = os.path.join(parent_dir, "csv")
    if not os.path.isdir(csv_dir_path):
        os.mkdir(csv_dir_path)
    return os.path.join(csv_dir_path, package_name)
...
def artifact_convert(doc_dir_path):
    ...
    for dir_name in os.listdir(doc_dir_path):
        ...
        csv_pack_dir_path = get_csv_writing_path(doc_dir_path)
...
def relation_convert(doc_dir_path, relation_dir):
    csv_pack_dir_path = get_csv_writing_path(doc_dir_path)
    if not os.path.isdir(csv_pack_dir_path):
        os.makedirs(csv_pack_dir_path)
